...
(
  texture <- readxl::read_excel(
    ...
    dplyr::mutate(survey_year = as.integer(survey_year)) |>
    dplyr::select(
      sample_number,
      survey_year,
      m_clay,
...
alpha <- soil_center_cores |>
  dplyr::mutate(
    source = "scc",
    survey_year = lubridate::year(sample_date)
  ) |>
  dplyr::full_join(
    y = texture |> dplyr::mutate(source = "texture"),
    by = c(
...
try({
  ...
  capeml::write_factors(soil_center_cores, overwrite = FALSE)
})
```
