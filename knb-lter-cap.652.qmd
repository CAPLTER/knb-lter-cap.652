...
try({ capeml::write_attributes(annuals, overwrite = FALSE) })
...
try({ capeml::write_attributes(number_perennials, overwrite = FALSE) })
...
neighborhood_characteristics <- get_neighborhood_characteristics("survey200") |>
  dplyr::mutate(
    neigh_social_class                          = as.factor(neigh_social_class),
...
try({ capeml::write_attributes(structures, overwrite = FALSE) })
...
try({ capeml::write_attributes(soil_center_cores, overwrite = FALSE) })
...
## locations as grid cells

```{r}
#| eval: TRUE
#| label: locations

try({capeml::write_attributes(cells_with_points, overwrite = FALSE)})

cells_with_points_desc <- "Geospatial file detailing the approximate locations of ESCA sampling sites in the CAP LTER study area encompassing the greater Phoenix metropolitan area and immediately surrounding Sonoran desert region. Polygons reflect a bounded area in which the study plot is located. This spatial resource is designed to provide the user with the general area of the study plot without disclosing the precise location. Precise sampling locations are available upon request to the CAP LTER Information Manager."

cells_with_points_SV <- capemlGIS::create_vector(
  vector_name = cells_with_points,
  description = cells_with_points_desc,
  driver      = "GeoJSON"
)
...
coverage <- EML::set_coverage(
  ...
  west                  = sf::st_bbox(cells_with_points)[["xmin"]],
  east                  = sf::st_bbox(cells_with_points)[["xmax"]],
  north                 = sf::st_bbox(cells_with_points)[["ymax"]],
  south                 = sf::st_bbox(cells_with_points)[["ymin"]]
)
