...
(
  texture <- readxl::read_excel(
    path  = "~/Dropbox/development/esca_working/soils/from_dropbox/compiled_texture_HH.xlsx",
    sheet = "All_Data_Calcs_OE_2015",
    skip  = 5
  ) |>
    janitor::clean_names() |>
    dplyr::filter(
      ...
      !is.na(survey_year),
      survey_year != 2000,
      # remove this depending on how you want to handle the miscodings but is
      # less of an issue with empty cases removed (i.e. last filter)
      !grepl(
        pattern     = "miscoded",
        x           = note,
        ignore.case = TRUE
      ...
      !(is.na(silt_fraction) & is.na(m_clay) & is.na(m_sand))
    ) |>
    dplyr::mutate(
      survey_year = as.integer(survey_year),
      silt_fraction = as.numeric(silt_fraction),
...
soil_center_cores <- soil_center_cores |>
  dplyr::mutate(survey_year = lubridate::year(sample_date)) |>
  dplyr::full_join(
    y = texture,
    by = c(
      "survey_year" = "survey_year",
      "site_code"   = "sample_number"
    )
  ) |>
  dplyr::mutate(
    notes = dplyr::case_when(
