...
# get all ESCA taxa, both vegetation and arthropod
esca_taxa <- bind_rows(
  dbGetQuery(pg, "
  SELECT
    itl.insect_scientific_name AS taxon
  FROM
    survey200.insect_taxon_list itl
    JOIN survey200.sweepnet_sample_insect_counts ssic ON ssic.insect_taxon_id = itl.insect_taxon_id
  GROUP BY
    itl.insect_scientific_name;") %>% 
    mutate(taxon = gsub("\\(immature\\)", "", taxon)) %>% 
    mutate(taxon = trimws(taxon, which = c("both"))),
  dbGetQuery(pg, "
  SELECT
    vtl.vegetation_scientific_name AS taxon
  FROM
    survey200.vegetation_taxon_list vtl
    JOIN survey200.vegetation_samples vs ON vs.vegetation_taxon_id = vtl.vegetation_taxon_id
  GROUP BY  
...
# convert tibble of taxa - esca_taxa was originally written to csv as I was
# trying to do this with taxonomyCleaner, which I could not make work, so here
# pulling in the file from that effort but this could be done with the output
# from the above query without writing to then reading from file.
esca_taxa <- read_csv('esca_taxa.csv') %>% 
  pull(taxon)
